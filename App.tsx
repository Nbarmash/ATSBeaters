...
const renderHelp = () => (
  ...
  const renderInput = () => {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
        ...
        <div className="bg-white p-5 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-xl border border-slate-100">
          ...
          <textarea 
            ...
            className="w-full h-48 md:h-64 p-4 md:p-6 bg-slate-50/50 border border-slate-100 rounded-2xl focus:ring-4 md:focus:ring-8 focus:ring-indigo-100 outline-none text-sm font-medium text-slate-700 leading-relaxed resize-none transition-all placeholder:text-slate-300"
          />
          {config.label2 && (
            <div className="mt-8">
              ...
              <textarea 
                ...
                className="w-full h-36 md:h-40 p-4 md:p-6 bg-slate-50/50 border border-slate-100 rounded-2xl focus:ring-8 focus:ring-indigo-100 outline-none text-sm font-medium text-slate-700 leading-relaxed resize-none transition-all placeholder:text-slate-300"
              />
            </div>
          )}
          <button 
        ...
        {state.result && activeTab !== AppTab.ANALYZER && (
           <div className="bg-white p-5 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-slate-100 animate-in fade-in slide-in-from-bottom-4">
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col selection:bg-indigo-100 selection:text-indigo-900">
      ...
      {/* Dynamic Navigation */}
      <header className="sticky top-0 z-50 glass border-b border-slate-100 px-4 md:px-12 py-5 flex items-center justify-between no-print">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-100 no-print" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        <div className="flex items-center justify-around px-1 py-2">
          ...
          <button onClick={() => { setActiveTab(AppTab.ANALYZER); handleReset(); setIsMobileMenuOpen(false); }} className={activeTab === AppTab.ANALYZER ? "flex flex-col items-center py-1.5 px-3 text-indigo-600" : "flex flex-col items-center py-1.5 px-3 text-slate-400"}>
            <i className="fas fa-microscope text-lg mb-0.5"></i>
            <span className="text-[10px] font-black">Analyze</span>
          </button>
          <button onClick={() => { setActiveTab(AppTab.REWRITE); handleReset(); setIsMobileMenuOpen(false); }} className={activeTab === AppTab.REWRITE ? "flex flex-col items-center py-1.5 px-3 text-indigo-600" : "flex flex-col items-center py-1.5 px-3 text-slate-400"}>
            <i className="fas fa-pen-to-square text-lg mb-0.5"></i>
            <span className="text-[10px] font-black">Rewrite</span>
          </button>
          <button onClick={() => { setActiveTab(AppTab.COVER_LETTER); handleReset(); setIsMobileMenuOpen(false); }} className={activeTab === AppTab.COVER_LETTER ? "flex flex-col items-center py-1.5 px-3 text-indigo-600" : "flex flex-col items-center py-1.5 px-3 text-slate-400"}>
            <i className="fas fa-envelope-open-text text-lg mb-0.5"></i>
            <span className="text-[10px] font-black">Cover</span>
          </button>
          <button onClick={() => { setActiveTab(AppTab.KEYWORDS); handleReset(); setIsMobileMenuOpen(false); }} className={activeTab === AppTab.KEYWORDS ? "flex flex-col items-center py-1.5 px-3 text-indigo-600" : "flex flex-col items-center py-1.5 px-3 text-slate-400"}>
            <i className="fas fa-magnifying-glass-chart text-lg mb-0.5"></i>
            <span className="text-[10px] font-black">Keywords</span>
          </button>
        </div>
      </nav>
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        <div
          ...
          style={{ clipPath: 'inset(0 round 2rem)' }}
        >
          <div className="absolute right-0 top-0 w-16 h-16 bg-indigo-50 rounded-full -z-10 opacity-60"></div>
          <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-2">Member Level</p>
          <p className="text-2xl md:text-3xl font-black text-slate-900 capitalize truncate">{user.tier}</p>
          <button onClick={() => setShowPricing(true)} className="mt-4 text-xs font-bold text-indigo-600 hover:underline">Manage Subscription →</button>
        </div>
