...
${result.formattingIssues.join(String.fromCharCode(10))}`;
    ...
    a.href = url; a.download = `ATSBeaters_Analysis_${Date.now()}.${format}`; a.click();
  };

  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">

      {/* Top 3 cards - stack on mobile */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        {/* Main Score */}
        <div className={`p-6 md:p-8 rounded-2xl md:rounded-3xl border ${getScoreColor(result.score)} flex flex-col items-center justify-center text-center shadow-sm relative overflow-hidden group`}>
          ...
          <p className="mt-4 text-sm font-bold opacity-80 flex items-center">
            <i className="fas fa-briefcase mr-2"></i> {result.suggestedJobField}
          </p>
        </div>

        {/* Growth Metric */}
        <div className="p-6 md:p-8 rounded-2xl md:rounded-3xl bg-slate-900 text-white flex flex-col items-center justify-center text-center shadow-xl border border-slate-800">
          <div className="w-10 h-10 md:w-12 md:h-12 bg-indigo-500 rounded-2xl flex items-center justify-center mb-3 md:mb-4 shadow-lg shadow-indigo-500/20">
            <i className="fas fa-arrow-trend-up text-lg md:text-xl"></i>
          </div>
          <span className="text-xs font-bold text-indigo-300 uppercase tracking-widest mb-1">Success Impact</span>
          <div className="text-4xl md:text-5xl font-black mb-2 text-white">{result.callbackImprovement}</div>
          <p className="text-xs text-slate-400 leading-relaxed px-2 md:px-4">Estimated increase in interview callbacks after implementing fixes</p>
        </div>

        {/* Action Controls */}
        <div className="p-6 md:p-8 rounded-2xl md:rounded-3xl bg-white border border-slate-200 flex flex-col justify-center shadow-sm space-y-2.5">
          <button onClick={onSave} className="w-full py-3 md:py-3.5 bg-indigo-600 text-white rounded-2xl font-black text-sm hover:bg-indigo-700 transition-all flex items-center justify-center shadow-lg shadow-indigo-600/10 active:scale-95">
          ...
          <button onClick={onReset} className="w-full py-2.5 text-xs font-black uppercase text-slate-400 border border-slate-100 rounded-xl hover:bg-slate-50 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
        <section className="bg-white p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-sm border border-slate-100">
          <h3 className="text-base md:text-lg font-black mb-4 md:mb-6 flex items-center text-slate-800">
            <span className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center mr-3 flex-shrink-0"><i className="fas fa-tags text-xs md:text-sm"></i></span>
        ...
        <section className="bg-white p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-sm border border-slate-100">
          <h3 className="text-base md:text-lg font-black mb-4 md:mb-6 flex items-center text-rose-600">
            <span className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-rose-100 text-rose-600 flex items-center justify-center mr-3 flex-shrink-0"><i className="fas fa-triangle-exclamation text-xs md:text-sm"></i></span>
          ...
          <ul className="space-y-3">
            {result.formattingIssues.map((issue, idx) => (
              <li key={idx} className="flex items-start text-sm text-slate-600 bg-rose-50/30 p-3 rounded-xl border border-rose-100/50">
                <i className="fas fa-circle-xmark text-rose-400 mt-0.5 mr-3 flex-shrink-0"></i>
      ...
      <section className="bg-white p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-sm border border-slate-100">
        <h3 className="text-base md:text-lg font-black mb-6 md:mb-8 flex items-center text-indigo-900">
          <span className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center mr-3 flex-shrink-0"><i className="fas fa-wand-sparkles text-xs md:text-sm"></i></span>
